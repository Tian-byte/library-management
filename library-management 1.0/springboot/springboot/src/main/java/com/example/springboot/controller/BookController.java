...
public class BookController {
    ...
    public Result uploadFile(MultipartFile file) {
        ...
        String filePath = BASE_FILE_PATH + System.currentTimeMillis() + flag +"_" + originalFilename;
        try{
            FileUtil.mkParentDirs(filePath);//创建父级目录
            file.transferTo(FileUtil.file(filePath));  //transferTo 可以将文件直接传入到系统的磁盘里面
           ...
           return Result.success("http://localhost:9090/api/book/file/download/" + flag + "?token=" + token);
        }catch (Exception e){
            log.info("文件上传失败",e);
        }
