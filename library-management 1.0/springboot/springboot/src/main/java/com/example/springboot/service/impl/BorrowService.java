...
public class BorrowService implements IBorrowService {
    ...
    public void save(Borrow obj) {
        ...
        Integer score = book.getScore() * obj.getDays();   // score = 用户借一本* 天数
        //4.校验用户积分
        if (score > account ){
        ...
        obj.setReturnDate(LocalDate.now().plus(obj.getDays(),ChronoUnit.DAYS));  //表示当前的日期 + days 得到归还的日期
        obj.setScore(score);
        //7.新增借书记录
    ...
    public void saveRetur(Retur obj) {
        //修改状态
        obj.setStatus("已归还");
        borrowMapper.updateStatus("已归还",obj.getId()); //obj.getId 前端传来的借书id
        //图书数量增加
       // obj.setId(null);  //新数据
        ...
        if (book != null){
            ...
            }else if (obj.getRealDate().isAfter(obj.getReturnDate())){//逾期归还，要扣除额外的积分
    ...
    public Map<String ,Object> getCountByTimeRange(String timeRange){
       ...
       // datetimeToDateStr  把DateTime 类型的List 转换成为一个String 的list
